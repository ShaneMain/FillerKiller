...
export function EpisodeRow({ episode, signedIn }: { episode: Episode; signedIn: boolean }) {
  ...
  const pct = score.fillerScore == null ? null : Math.round(score.fillerScore * 100);

  return (
    <div className="flex items-center gap-3 border-b border-zinc-800 py-3">
      <div className="w-12 shrink-0 text-center text-sm text-zinc-500">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-xs text-zinc-500">
          {score.fillerVotes + score.canonVotes === 0
            ? "No votes yet"
            : `${pct}% filler · ${score.fillerVotes + score.canonVotes} votes`}
      ...
      <div className="flex shrink-0 gap-1.5">
        <VoteButton
          label="Filler"
          active={score.myVote === "FILLER"}
          activeCls="bg-rose-600 text-white"
          disabled={!signedIn || busy}
          onClick={() => void vote("FILLER")}
        />
        <VoteButton
          label="Canon"
          active={score.myVote === "CANON"}
          activeCls="bg-emerald-600 text-white"
          disabled={!signedIn || busy}
          onClick={() => void vote("CANON")}
        />
      </div>
    </div>
  );
